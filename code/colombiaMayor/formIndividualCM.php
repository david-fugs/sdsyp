...
if ($tipo_usuario == 1) {
    ...
    $query_usuarios = "SELECT id, nombre, tipo_usuario FROM usuarios WHERE tipo_usuario IN (1, 8, 9) ORDER BY nombre ASC";
    ...
    $query_usuarios = "SELECT id, nombre, tipo_usuario FROM usuarios WHERE id = $usuario_id";
} else {
    $query_usuarios = "SELECT id, nombre, tipo_usuario FROM usuarios WHERE id = $usuario_id";
}
...
<body>
        ...
        <div class="modern-container">
            ...
            <div class="modern-filters">
                <form action="formIndividualCM.php" method="get" class="filter-row">
                    ...
                    <div class="filter-group">
                        <label for="filter_tipo_usuario">Tipo de Usuario</label>
                        <select name="filtro_tipo_usuario" id="filter_tipo_usuario" class="modern-select">
                            <option value="">-- Todos --</option>
                            <option value="8" <?= (isset($_GET['filtro_tipo_usuario']) && $_GET['filtro_tipo_usuario'] == 8) ? 'selected' : '' ?>>Técnico Colombia Mayor</option>
                            <option value="9" <?= (isset($_GET['filtro_tipo_usuario']) && $_GET['filtro_tipo_usuario'] == 9) ? 'selected' : '' ?>>Contratista Colombia Mayor</option>
                        </select>
                    </div>
                    <?php endif; ?>
                    <div class="filter-group">
    ...
    <script>
        ...
        // Función para mostrar texto completo en celda truncada
        function verMasCM(btn) {
            const full = btn.getAttribute('data-full');
            const span = btn.parentElement;
            span.innerHTML = full + ' <button type="button" class="btn-ver-mas-cm" onclick="verMenosCM(this)" data-short="' + span.innerHTML.replace(/"/g, '&quot;') + '"><i class=\"bi bi-eye-slash-fill\"></i> menos</button>';
        }

        // No usamos verMenosCM explícitamente, SweetAlert es más limpio:
        function verMasCM(btn) {
            Swal.fire({
                title: 'Información completa',
                text: btn.getAttribute('data-full'),
                icon: 'info',
