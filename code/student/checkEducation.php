...
<body>
    ...
    <section class="principal">
        ...
        <div style="border-radius: 9px 9px 9px 9px; -moz-border-radius: 9px 9px 9px 9px; -webkit-border-radius: 9px 9px 9px 9px; border: 4px solid #FFFFFF;" align="center">
            ...
            <div align="center">
                <h1 style="color: #412fd1; text-shadow: #FFFFFF 0.1em 0.1em 0.2em"><b><i class="fa-solid fa-baby"></i> ENCUESTAS APLICADAS y/o REALIZADAS</b></h1>
            </div>

            <div class="flex">
                <div class="box">
                    <form action="checkEducation.php" method="get">
            ...
            <div class="d-flex justify-content-center ">
                ...
                <a class="ml-4" href="exportar/exportarAllEducation.php"><img src='../../img/excel.png' width="75" height="80" title="Regresar" /></a>
        ...
        <div class='card-body'>
            ...
            $consulta = "SELECT educacion.*, estudiantes.*, ie.*, COUNT(educacion.num_doc_est) as veces_aplicada 
             FROM educacion 
             INNER JOIN estudiantes ON educacion.num_doc_est = estudiantes.num_doc_est 
             INNER JOIN ieSede ON estudiantes.cod_dane_ieSede = ieSede.cod_dane_ieSede 
             INNER JOIN ie ON ieSede.cod_dane_ie = ie.cod_dane_ie 
             WHERE (estudiantes.num_doc_est LIKE '%$num_doc_est%') 
             AND (estudiantes.nom_ape_est LIKE '%$nom_ape_est%') 
             AND (estudiantes.grado_est LIKE '%$grado_est%')
             AND fechacreacion_educacion >= '2023-10-01' 
             AND ie.cod_dane_ie = $cod_dane_ie 
             GROUP BY educacion.num_doc_est 
             ORDER BY educacion.num_doc_est ASC 
            ...
            <tr>
                ...
                <td data-label="APLICADA EL">' . $row['fechacreacion_educacion'] . '</td>
                <td data-label="REALIZÓ">' . utf8_encode($row['nombre_encuestador_educacion']) . '</td>
                <td data-label="MODIFICADA EL">' . $row['fechaedicion_educacion'] . '</td>
                <td data-label="VECES APLICADA" class="' . $veces_clase . '">' . $row['veces_aplicada'] . '</td>
                <td data-label="VER ENCUESTAS"><a href="viewEducationSurvey.php?num_doc_est=' . $row['num_doc_est'] . '"><img src="../../img/search.png" width=28 height=28></a></td>
                  <td data-label="EXPORTAR"><a href="exportar/exportarSurveysEducation.php?num_doc_est=' . $row['num_doc_est'] . '"><img src="../../img/excel.png" width=32 height=32></a></td>
