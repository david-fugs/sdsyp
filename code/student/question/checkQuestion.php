...
<body>
    ...
    <section class="principal">
        ...
        <div style="border-radius: 9px 9px 9px 9px; -moz-border-radius: 9px 9px 9px 9px; -webkit-border-radius: 9px 9px 9px 9px; border: 4px solid #FFFFFF;" align="center">
            ...
            <div align="center">
                <h1 style="color: #412fd1; text-shadow: #FFFFFF 0.1em 0.1em 0.2em"><b> <i class="fa-sharp fa-solid fa-book"></i>
                        ENCUESTAS APLICADAS y/o REALIZADAS</b></h1>
            </div>

            <div class="flex">
                <div class="box">
                    <form action="checkQuestion.php" method="get">
            ...
            <div class="d-flex justify-content-center ">
                ...
                <a class="ml-4" href="../exportar/exportarAllQuestion.php"><img src='../../../img/excel.png' width="75" height="80" title="Regresar" /></a>
            </div>
            <?php
            ...
            $query = "SELECT preguntas.*, estudiantes.*, ie.*, COUNT(preguntas.num_doc_est) as veces_aplicada 
          FROM preguntas 
          ...
          AND fecha_dig_preguntas >= '2023-10-01' 
          AND ie.cod_dane_ie = $cod_dane_ie 
          GROUP BY preguntas.num_doc_est 
          ORDER BY preguntas.num_doc_est ASC";
        ...
        <div class='card-body'>
            ...
            $consulta = "SELECT preguntas.*, estudiantes.*, ie.*, COUNT(preguntas.num_doc_est) as veces_aplicada 
             FROM preguntas 
             INNER JOIN estudiantes ON preguntas.num_doc_est = estudiantes.num_doc_est 
             INNER JOIN ieSede ON estudiantes.cod_dane_ieSede = ieSede.cod_dane_ieSede 
             INNER JOIN ie ON ieSede.cod_dane_ie = ie.cod_dane_ie 
    ...
    <tr>
        ...
        <td data-label="APLICADA EL">' . $row['fecha_dig_preguntas'] . '</td>
        <td data-label="REALIZÓ">' . utf8_encode($row['nombre_encuestador_preguntas']) . '</td>
        <td data-label="MODIFICADA EL">' . $row['fechaedicion_preguntas'] . '</td>
        <td data-label="VECES APLICADA" class="' . $veces_clase . '">' . $row['veces_aplicada'] . '</td>
        <td data-label="VER ENCUESTAS"><a href="viewQuestionSurvey.php?num_doc_est=' . $row['num_doc_est'] . '"><img src="../../../img/search.png" width=28 height=28></a></td>    
        <td data-label="EXPORTAR"><a href="../exportar/exportarSurveyspreguntas.php?num_doc_est=' . $row['num_doc_est'] . '"><img src="../../../img/excel.png" width=32 height=32></a></td>
        </tr>';
            }
