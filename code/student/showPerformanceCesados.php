    ...
    <script>
         function cambiarEstado(num_doc_est) {
            if (confirm('¿Está seguro que desea cambiar el estado del estudiante?')) {
                window.location.href = 'cambiarEstado.php?num_doc_est=' + num_doc_est + '&campo=' + 'desempeno' + '&valor=' + 0;
            }
        }

        function cesados() {
            window.location.href = 'showPerformance.php';
        }
    </script>
</head>
    ...
    <section class="principal">
        ...
        $query = "SELECT estudiantes.*, usuarios.*, ie.*
          ...
          AND (estudiantes.grado_est LIKE '%$grado_est%')
            AND estudiantes.estado_desempeno = 1  
