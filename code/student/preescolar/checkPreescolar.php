...
<body>
    ...
    <section class="principal">
        ...
        <div style="border-radius: 9px 9px 9px 9px; -moz-border-radius: 9px 9px 9px 9px; -webkit-border-radius: 9px 9px 9px 9px; border: 4px solid #FFFFFF;" align="center">
            ...
            <div class="flex">
                <div class="box">
                    <form action="checkPreescolar.php" method="get">
            ...
            <div class="d-flex justify-content-center " >
                ...
                <a  class="ml-4" href="../exportar/exportarAllPreescolar.php"><img src='../../../img/excel.png' width="75" height="80" title="Regresar" /></a>
            </div>
            <?php
        ...
        <div class='card-body'>
            ...
            $consulta = "SELECT preescolar.*, estudiantes.*, ie.*, COUNT(preescolar.num_doc_est) as veces_aplicada 
             FROM preescolar 
             INNER JOIN estudiantes ON preescolar.num_doc_est = estudiantes.num_doc_est 
             INNER JOIN ieSede ON estudiantes.cod_dane_ieSede = ieSede.cod_dane_ieSede 
             INNER JOIN ie ON ieSede.cod_dane_ie = ie.cod_dane_ie 
             WHERE (estudiantes.num_doc_est LIKE '%$num_doc_est%') 
             AND (estudiantes.nom_ape_est LIKE '%$nom_ape_est%') 
             AND (estudiantes.grado_est LIKE '%$grado_est%')
             AND ie.cod_dane_ie = $cod_dane_ie 
             GROUP BY preescolar.num_doc_est 
             ORDER BY preescolar.num_doc_est ASC 
    ...
    <tr>
        ...
        <td data-label="APLICADA EL">' . $row['fecha_dig_preescolar'] . '</td>
        <td data-label="REALIZÓ">' . utf8_encode($row['nombre_encuestador_preescolar']) . '</td>
        <td data-label="MODIFICADA EL">' . $row['fechaedicion_preescolar'] . '</td>
        <td data-label="VECES APLICADA" class="' . $veces_clase . '">' . $row['veces_aplicada'] . '</td>
        <td data-label="VER ENCUESTAS"><a href="viewPreescolarSurvey.php?num_doc_est=' . $row['num_doc_est'] . '"><img src="../../../img/search.png" width=28 height=28></a></td>    
        <td data-label="EXPORTAR"><a href="../exportar/exportarSurveysPreescolar.php?num_doc_est=' . $row['num_doc_est'] . '"><img src="../../../img/excel.png" width=32 height=32></a></td>
        </tr>';
            }
