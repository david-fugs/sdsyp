    ...
    <body>
		...
		<section class="principal">
			...
			<div style="border-radius: 9px 9px 9px 9px; -moz-border-radius: 9px 9px 9px 9px; -webkit-border-radius: 9px 9px 9px 9px; border: 4px solid #FFFFFF;" align="center">
				...
				<div align="center">
					<h1 style="color: #412fd1; text-shadow: #FFFFFF 0.1em 0.1em 0.2em"><b><i class="fas fa-users"></i> INFORMACIÓN GENERAL DEL ESTUDIANTE</b></h1>
				</div>
    			...
    			<div style="border-radius: 9px 9px 9px 9px; -moz-border-radius: 9px 9px 9px 9px; -webkit-border-radius: 9px 9px 9px 9px; border: 1px solid #efd47d; width: 500px; height: 30px; background:#FAFAFA; display:table-cell; vertical-align:middle;">
	    			...
	    			<form action="showstudent.php" method="get">
...
<?php
	...
	$query = "SELECT * FROM `estudiantes` INNER JOIN `ieSede` ON estudiantes.cod_dane_ieSede=ieSede.cod_dane_ieSede INNER JOIN ie ON ieSede.cod_dane_ie=ie.cod_dane_ie WHERE (num_doc_est LIKE '%".$num_doc_est."%') AND (nom_ape_est LIKE '%".$nom_ape_est."%') AND ie.cod_dane_ie=$cod_dane_ie ORDER BY num_doc_est ASC";
	...
	$consulta = "SELECT * FROM `estudiantes` INNER JOIN `ieSede` ON estudiantes.cod_dane_ieSede=ieSede.cod_dane_ieSede INNER JOIN ie ON ieSede.cod_dane_ie=ie.cod_dane_ie WHERE (num_doc_est LIKE '%".$num_doc_est."%') AND (nom_ape_est LIKE '%".$nom_ape_est."%') AND ie.cod_dane_ie=$cod_dane_ie ORDER BY num_doc_est ASC LIMIT " .(($paginacion->get_page() - 1) * $resul_x_pagina). "," .$resul_x_pagina;
	...
	while($row = mysqli_fetch_array($result))
	{
		...
		echo '
				<tr>
					...
					<td data-label="ESTUDIANTE">'.utf8_encode($row['obs_est']).'</td>
					<td data-label="EDITAR"><a href="addstudentedit.php?num_doc_est='.$row['num_doc_est'].'"><img src="../../img/editar.png" width=28 height=28></a></td>
