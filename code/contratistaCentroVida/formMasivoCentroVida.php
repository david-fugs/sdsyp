...
if(isset($_GET['delete'])){
    ...
    if($del>0){
        ...
        if($stmtDel){ $stmtDel->bind_param('i',$del); $stmtDel->execute(); $stmtDel->close(); }
        echo "<script>window.location='formMasivoCentroVida.php';</script>";exit;
    }
}
if ($filtro_anio) { $where .= " AND YEAR(ra.fecha_atencion) = $filtro_anio"; }
if ($filtro_mes) { $where .= " AND MONTH(ra.fecha_atencion) = $filtro_mes"; }
if ($filtro_funcionario) { $where .= " AND ra.id_usuario = $filtro_funcionario"; }
...
<style>
body{font-size:16px;background:#f8fafc}
.modern-container{background:#fff;border-radius:12px;box-shadow:0 4px 6px -1px rgba(0,0,0,.1);overflow:hidden;margin:20px auto;max-width:1400px}
.modern-header{background:linear-gradient(135deg,#e91e63,#9c27b0);color:#fff;padding:24px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px}
.modern-header h2{margin:0;font-size:24px;font-weight:600}
.btn-modern{background:rgba(255,255,255,.2);border:2px solid rgba(255,255,255,.3);color:#fff;padding:10px 18px;border-radius:8px;cursor:pointer;transition:.3s}
.btn-modern:hover{background:rgba(255,255,255,.3);border-color:rgba(255,255,255,.5);transform:translateY(-2px);color:#fff}
.modern-filters{padding:20px;background:#f8fafc;border-bottom:1px solid #e5e7eb}
...
.modern-input,.modern-select{border:1px solid #d1d5db;border-radius:6px;padding:8px 12px;font-size:14px}
.modern-table-wrapper{overflow-x:auto}
.modern-table{width:100%;border-collapse:collapse;font-size:13px}
.modern-table th{background:#f8fafc;color:#374151;font-weight:600;padding:10px 8px;border-bottom:2px solid #e5e7eb;white-space:nowrap}
.modern-table td{padding:8px 8px;border-bottom:1px solid #f3f4f6;vertical-align:middle;white-space:nowrap;max-width:180px;overflow:hidden;text-overflow:ellipsis}
.action-buttons{display:flex;gap:6px;justify-content:center}
.btn-action{border:none;border-radius:4px;padding:6px 10px;cursor:pointer;font-size:12px;display:inline-flex;align-items:center;justify-content:center;transition:.2s}
.btn-edit{background:#3b82f6;color:#fff}.btn-edit:hover{background:#2563eb}
...
<body>
    ...
    <div class="container mt-5">
        <div class="modern-container">
            ...
            document.addEventListener('DOMContentLoaded',()=>{
              const sync=()=>{export_filtro_anio.value=filtro_anio.value;export_filtro_mes.value=filtro_mes.value;export_filtro_funcionario.value=filtro_funcionario.value;};
              ['change'].forEach(ev=>['filtro_anio','filtro_mes','filtro_funcionario'].forEach(id=>document.getElementById(id).addEventListener(ev,sync)));sync();
            });
            </script>
                ...
                <form action="formMasivoCentroVida.php" method="get" class="filter-row">
                    <div class="filter-group">
                        ...
                        <select id="filtro_anio" name="filtro_anio" class="modern-select">
                            ...
                            <?php $currentYear=date('Y'); for($y=2023;$y<=$currentYear;$y++){ $sel=$filtro_anio==$y?'selected':''; echo "<option value='$y' $sel>$y</option>"; } ?>
                        </select>
                    </div>
                    <div class="filter-group">
                        ...
                        <select id="filtro_mes" name="filtro_mes" class="modern-select">
                            <option value="">Todos</option>
                            <?php $meses=[1=>'Enero',2=>'Febrero',3=>'Marzo',4=>'Abril',5=>'Mayo',6=>'Junio',7=>'Julio',8=>'Agosto',9=>'Septiembre',10=>'Octubre',11=>'Noviembre',12=>'Diciembre']; foreach($meses as $num=>$nom){$sel=$filtro_mes==$num?'selected':''; echo "<option value='$num' $sel>$nom</option>";} ?>
                        </select>
                    </div>
                    <div class="filter-group">
                        ...
                        <select id="filtro_funcionario" name="filtro_funcionario" class="modern-select">
                            <option value="">Todos</option>
                            <?php if($usuarios){ while($u=$usuarios->fetch_assoc()){ $sel=$filtro_funcionario==$u['id']?'selected':''; echo "<option value='{$u['id']}' $sel>".htmlspecialchars($u['nombre'])."</option>"; }} ?>
                        </select>
                    </div>
                    <div class="filter-group">
                ...
                <table class="modern-table" id="tabla">
                    ...
                    <tbody>
                        ...
                        <?php } } else { 
                            ...
                            for($i=0;$i<18;$i++){ echo "<td class='text-muted'></td>"; }
                            echo "</tr>"; 
                        } ?>
                    </tbody>
                </table>
    ...
    <br><center><a href='../../access.php'><img src='../../img/atras.png' width='72' height='72'></a></center><br>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3 form-floating">
                                <select class="form-select" id="meta" name="id_meta" required>
                                    ...
                                    <?php if($metas){ mysqli_data_seek($metas,0); while($m=$metas->fetch_assoc()){ echo "<option value='{$m['id_meta']}'>".htmlspecialchars($m['descripcion_meta'])."</option>"; } } ?>
                                </select>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3 form-floating">
                                <select class="form-select" id="actividad_centro_vida" name="id_actividad_centro_vida" required>
                                    ...
                                    <?php if($actividades_cv){ mysqli_data_seek($actividades_cv,0); while($acv=$actividades_cv->fetch_assoc()){ echo "<option value='{$acv['id_actividad_centro_vida']}'>".htmlspecialchars($acv['descripcion_actividad'])."</option>"; } } ?>
                                </select>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3 form-floating">
                                <select class="form-select" id="centro_vida" name="id_centro_vida">
                                    <option value="" selected>Seleccione...</option>
                                    <?php if($grupos){ while($g=$grupos->fetch_assoc()){ echo "<option value='{$g['id_grupo']}'>".htmlspecialchars($g['descripcion_grupo'])."</option>"; } } ?>
                                </select>
                            ...
                            <div class="col-md-4 mb-3 form-floating">
                                <select name="funcionario_responsable" id="funcionario_responsable" class="form-select">
                                    ...
                                    <?php mysqli_data_seek($usuarios,0); if($usuarios){ while($u=$usuarios->fetch_assoc()){ echo "<option value='{$u['id']}'>".htmlspecialchars($u['nombre'])."</option>"; } } ?>
                                </select>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3 form-floating">
                                <select name="id_comuna" id="id_comuna" class="form-select">
                                    <option value="" selected>Seleccione...</option>
                                    <?php if($comunas){ while($c=$comunas->fetch_assoc()){ echo "<option value='{$c['id_com']}'>".htmlspecialchars($c['nombre_com'])."</option>"; } } ?>
                                </select>
...
$(function(){
  ...
  if($.fn.DataTable){
      ...
      $('#tabla tbody tr').each(function(){
          if($(this).find('td').length !== headerCols){ valid=false; }
      });
      if(valid){
          $('#tabla').DataTable({
              pageLength:15,
  ...
  $('#modalAdd').on('show.bs.modal', function(e){
      ...
      if(metaId){
          $actividad.prop('disabled', true).html('<option value="">Cargando...</option>');
          $.post('../contratista/getActividades.php',{id_meta:metaId}, function(r){
                $actividad.html('<option value="">Seleccione Actividad...</option>'+r).prop('disabled', false);
                if(actividadId){
                     $actividad.val(actividadId);
                     $accion.prop('disabled', true).html('<option value="">Cargando...</option>');
                     $.post('../contratista/getAcciones.php',{id_actividad:actividadId}, function(r2){
                          $accion.html('<option value="">Seleccione Acción...</option>'+r2).prop('disabled', false);
                          if(accionId){
                                $accion.val(accionId);
                                // Políticas
                                $politica.prop('disabled', true).html('<option value="">Cargando...</option>');
                                $.ajax({url:'../personMovement/getPoliticaPublica.php',type:'POST',data:{id_accion:accionId},dataType:'json',success:function(resp){
                                     $politica.html('<option value="">Seleccione Política Pública...</option>');
                                     if(resp && resp.politicas){ resp.politicas.forEach(p=>$politica.append('<option value="'+p.id_politica+'">'+p.descripcion_politica+'</option>')); }
                                     if(politicaId){ $politica.val(politicaId); }
                                     $politica.prop('disabled', false);
                                },error:function(){ $politica.html('<option value="">Error al cargar</option>').prop('disabled', false); }});
                          }
                     });
                }
          });
