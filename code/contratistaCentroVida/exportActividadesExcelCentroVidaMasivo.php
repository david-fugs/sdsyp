<?php
// Exportar actividades masivas centro vida (2 hojas: Mañana y Tarde)
session_start();
...
if (debeAplicarFiltroGrupo($_SESSION['tipo_usuario'] ?? null) && isset($_SESSION['id_grupo'])) {
    ...
    $where_base .= " AND mcv.id_centro_vida = $id_grupo";
}
...
$headers=['Fecha Atención','Meta','Actividad Plan','Acción','Política Pública','Centro Vida','Actividad Centro Vida','Nombre Líder','Teléfono','Comuna','Medio Verificación','Masculino','Femenino','Total','Jornada','Profesión','Tipo Actividad','Observación','Digitado por','Funcionario Responsable'];
$lastCol=\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex(count($headers));

function llenarHojaExportMasivo($sheet, $res, $headers, $lastCol) {
    $col='A';
    ...
    $r=2;
    if($res){ while($row=$res->fetch_assoc()){ $data=[
        $row['fecha_atencion'],$row['descripcion_meta'],$row['descripcion_actividad'],$row['descripcion_accion'],$row['descripcion_politica'],
        $row['centro_vida'],$row['actividad_centro_vida'],$row['nombre_lider'],$row['telefono_contacto'],$row['nombre_comuna'],$row['medio_verificacion'],
        $row['cantidad_masculino'],$row['cantidad_femenino'],($row['cantidad_masculino']+$row['cantidad_femenino']),$row['jornada'],$row['profesion'],$row['tipo_actividad'],$row['observacion_actividad'],$row['digitado_por'],$row['funcionario_responsable_nombre']];
    ...
    for($i=1;$i<=20;$i++){ $sheet->getColumnDimension(\PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($i))->setWidth(25); }
...
$whereM = $where_base . " AND mcv.jornada = 'Mañana' ORDER BY mcv.fecha_atencion DESC";
$resM = $mysqli->query($sql_base . $whereM);
llenarHojaExportMasivo($sheetM, $resM, $headers, $lastCol);

// ── Hoja Tarde
$sheetT = $spreadsheet->createSheet();
$sheetT->setTitle('Tarde');
$whereT = $where_base . " AND mcv.jornada = 'Tarde' ORDER BY mcv.fecha_atencion DESC";
$resT = $mysqli->query($sql_base . $whereT);
llenarHojaExportMasivo($sheetT, $resT, $headers, $lastCol);

// ── Hoja Sin Jornada
$sheetS = $spreadsheet->createSheet();
$sheetS->setTitle('Sin Jornada');
$whereS = $where_base . " AND (mcv.jornada IS NULL OR mcv.jornada = '') ORDER BY mcv.fecha_atencion DESC";
$resS = $mysqli->query($sql_base . $whereS);
llenarHojaExportMasivo($sheetS, $resS, $headers, $lastCol);
